...
object BmwJobs {
    ...
    private val keyReadingJobs = listOf(
        ...
        // Direct memory read jobs — read CAS/EWS/FEM EEPROM without key present
        BmwJob(
            id = "cas_direct_read_isn",
            label = "Direct read CAS ISN (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xA0, label = "Read ISN from CAS memory")),
            description = "Read ISN directly from CAS EEPROM. Works even with no key inserted.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_key_memory",
            label = "Direct read CAS key memory (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xA1, label = "Read key memory block")),
            description = "Read all key slot data directly from CAS EEPROM. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_vin",
            label = "Direct read CAS VIN (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xA2, label = "Read VIN from CAS memory")),
            description = "Read VIN directly from CAS EEPROM. Always available.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "ews_direct_read_memory",
            label = "Direct read EWS memory (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xA3, label = "Read EWS memory block")),
            description = "Read EWS key and ISN data directly from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.DME.name),
        ),
        BmwJob(
            id = "fem_direct_read_memory",
            label = "Direct read FEM/BDC memory (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xA4, label = "Read FEM memory block")),
            description = "Read FEM/BDC key data directly from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_slot_1",
            label = "Direct read CAS slot 1 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB0, label = "Read CAS slot 1 EEPROM")),
            description = "Read key data for slot 1 from CAS EEPROM directly. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_slot_2",
            label = "Direct read CAS slot 2 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB1, label = "Read CAS slot 2 EEPROM")),
            description = "Read key data for slot 2 from CAS EEPROM directly. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_slot_3",
            label = "Direct read CAS slot 3 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB2, label = "Read CAS slot 3 EEPROM")),
            description = "Read key data for slot 3 from CAS EEPROM directly. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "cas_direct_read_slot_4",
            label = "Direct read CAS slot 4 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB3, label = "Read CAS slot 4 EEPROM")),
            description = "Read key data for slot 4 from CAS EEPROM directly. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "ews_direct_read_slot_1",
            label = "Direct read EWS slot 1 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB4, label = "Read EWS slot 1 memory")),
            description = "Read key data for EWS slot 1 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.DME.name),
        ),
        BmwJob(
            id = "ews_direct_read_slot_2",
            label = "Direct read EWS slot 2 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB5, label = "Read EWS slot 2 memory")),
            description = "Read key data for EWS slot 2 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.DME.name),
        ),
        BmwJob(
            id = "ews_direct_read_slot_3",
            label = "Direct read EWS slot 3 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB6, label = "Read EWS slot 3 memory")),
            description = "Read key data for EWS slot 3 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.DME.name),
        ),
        BmwJob(
            id = "fem_direct_read_slot_1",
            label = "Direct read FEM slot 1 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB7, label = "Read FEM slot 1 memory")),
            description = "Read key data for FEM slot 1 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "fem_direct_read_slot_2",
            label = "Direct read FEM slot 2 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB8, label = "Read FEM slot 2 memory")),
            description = "Read key data for FEM slot 2 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "fem_direct_read_slot_3",
            label = "Direct read FEM slot 3 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xB9, label = "Read FEM slot 3 memory")),
            description = "Read key data for FEM slot 3 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
        BmwJob(
            id = "fem_direct_read_slot_4",
            label = "Direct read FEM slot 4 (no key required)",
            category = JobCategory.IDENTIFICATION,
            steps = standardSessionPack() + listOf(step(0x21, 0xBA, label = "Read FEM slot 4 memory")),
            description = "Read key data for FEM slot 4 from module memory. No key required.",
            supportedTargets = setOf(BmwTargets.CAS.name),
        ),
    )
